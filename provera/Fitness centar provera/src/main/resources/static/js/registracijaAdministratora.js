$(document).on("submit","form",function(event){		 // kada je submit-ovana forma za kreiranje novog zaposlenog
	...
	var newAdministratorDTORegJSON=formToJSON(korisnickoIme,lozinka,ime,prezime,kontakt_telefon,email,datum_rodjenja);
	
	
	// ajax poziv za kreiranje novog clana na backend-u
	$.ajax({
		type:"POST",
		url:"http://localhost:8080/api/administrator",
		dataType:"json",
